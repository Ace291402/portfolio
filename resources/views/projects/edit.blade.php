...
<body class="bg-slate-950 text-white min-h-screen">
    <header class="sticky top-0 z-50 bg-slate-900 border-b border-slate-700">
        <div class="max-w-4xl mx-auto flex items-center justify-between px-6 py-4">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-violet-300">Edit Project</h1>
            </div>
    ...
    <main class="max-w-3xl mx-auto px-6 py-8">
        ...
        <div class="mb-8">
            <h2 class="text-3xl font-bold mb-2">Edit Project</h2>
            <p class="text-slate-400">Update your project and keep admin/dashboard in sync.</p>
        </div>

        <form class="space-y-6" method="POST" action="{{ route('projects.update', $project) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <label class="relative cursor-pointer border border-slate-700 rounded-xl bg-slate-900 aspect-video flex items-center justify-center overflow-hidden hover:border-violet-400 transition">
                    ...
                    <img id="preview" class="absolute inset-0 w-full h-full object-cover opacity-0" src="{{ $project->image ? asset('storage/' . $project->image) : '' }}" />
                    <div id="placeholder" class="flex flex-col items-center gap-3 z-10 {{ $project->image ? 'hidden' : '' }}">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="project-title" name="title" value="{{ old('title', $project->title) }}" placeholder="Enter project title" class="w-full rounded-xl bg-slate-900 border border-slate-700 px-4 py-3 focus:border-violet-400 focus:ring-0" required>
            </div>

            <div class="space-y-2">
                <label for="project-desc" class="block text-sm text-slate-300">Description</label>
                <textarea id="project-desc" name="description" rows="5" placeholder="Describe your project..." class="w-full rounded-xl bg-slate-900 border border-slate-700 px-4 py-3 resize-none focus:border-violet-400 focus:ring-0">{{ old('description', $project->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-2">
                    <label for="project-date" class="block text-sm text-slate-300">Target Date</label>
                    <input type="date" id="project-date" name="target_date" value="{{ old('target_date', optional($project->target_date)->format('Y-m-d')) }}" class="w-full rounded-xl bg-slate-900 border border-slate-700 px-4 py-3 focus:border-violet-400 focus:ring-0">
                </div>
                <div class="space-y-2">
            ...
            <section class="space-y-3">
                ...
                <input type="hidden" name="skills" id="skills-data" value="{{ old('skills', json_encode($project->skills ?? [])) }}">
            </section>

            <div class="pt-4 space-y-3">
                <button type="submit" class="w-full py-4 rounded-xl bg-violet-500 hover:bg-violet-600 font-semibold transition">Update Project</button>
